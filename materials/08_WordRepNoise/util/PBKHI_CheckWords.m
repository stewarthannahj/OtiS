...
num_Words = length(STIM.Word)
for word_Num = 1:num_Words
    targetWav = sprintf('%s%s%s','C:\projects\PBK_HI\RajMWC_ALL_Fixed_Equalized\',STIM.Word(word_Num).str,'.wav');
    if ~exist(targetWav,'file')
        x = STIM.Word(word_Num).str;
        display_String = sprintf('%s %s','Missing',x);
        disp(display_String)
    end
end
...
file_Search_String = strcat('C:\projects\PBK_HI\RajMWC_ALL_Fixed_Equalized\','*.wav');
...
for k = 1:num_Files
    ...
    extraWord = true;
    for word_Num = 1:num_Words
        wavName = strcat(STIM.Word(word_Num).str,'.wav');
        if strcmp(wavName,char(wav_File_Name_Trimmed{k}))
            extraWord = false;
            break;
        end
    end
    if extraWord
        display_String = sprintf('%s %s','Extra',char(wav_File_Name_Trimmed{k}));
        disp(display_String)
    end
end
...
TRACK.AllPlayed={};
TRACK.AllPause=[];
TRACK.Ceiling=90;				% max signal level allowed
...
while isempty(STIM.MaskerCond),
    disp(' ');
    disp('Select masker condition (Do NOT run quiet here): ')
...
switch STIM.MaskerCond
    ...
    case 'SSN Amplitude-Modulated'
        e(1)=invoke(IO.r,'SetTagVal','MskChannel',5);        
    case '2-talker'
        e(1)=invoke(IO.r,'SetTagVal','MskChannel',2);
    case '2-talker Time-Reversed'
        e(1)=invoke(IO.r,'SetTagVal','MskChannel',4);
    case 'quiet'
        e(1)=invoke(IO.r,'SetTagVal','MskChannel',0);
    otherwise
...
FileName=[IO.Root,IO.StimFolderAbsent,WordPlayed, '.wav'];
y=wavread(FileName);
y=y';
