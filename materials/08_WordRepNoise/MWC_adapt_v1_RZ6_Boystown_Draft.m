...
function MWC_adapt_v1_RZ6_Boystown_Draft

clear										% clears all variables

[IO,STIM]=SetConstants;             % sets all constants. References separate script.
...
[IO,player1,player2,yarray1,yarray2,Fs1,Fs2,speakerIDMask]=SetUpTDT(IO,STIM,TRACK);         % set up tdt hardware w/ those variables
...
while not(TRACK.EndTrack),
    ...
    [Resp,RespRight,TRACK]=GetResp(TRACK);        % get observer response
    [TRACK,IO]=RespEval(IO,TRACK,Resp,RespRight);	% assess resp. & adjust signal lev
    
    %LevScalar = (10^((TRACK.MaskerLev)/20))/(10^(75.4/20));	%% compute tone amplitude scaler
    
    %LevScalar = (10^((TRACK.MaskerLev)/20))/(10^(56.5/20));
    
    
    [player1,player2]=UpdateMaskLev(yarray1,yarray2,Fs1,Fs2,speakerIDMask,TRACK,ti1,ti2);
...
disp([' Preparing to run ',IO.Prog,': ']);                                  %% Have not changed from original MATLAB code. Not sure what the set up for Boystown fully entails.
...
disp(' Set computer level to 50 for all audo devices involved (Speakers 1, 5, 6, 8)'); %% Added to make sure computer levels are set to the correct value.
disp('  ');
end
%------------------------------------------------------------------%
...
TRACK.Correct={};               % Record if tester heard right 
end
%------------------------------------------------------------------%
...
%------------------------------------------------------------------%
function [IO,player1,player2,y1,y2,Fs1,Fs2,speakerIDMask]=SetUpTDT(IO,STIM,TRACK)

% TDT ActiveX calls
%IO.r = 'Windows DirectSound';		% look for RP2 & assign shortcut to 'IO.r' %% Changed to a different driver
%c=invoke(IO.r, 'ConnectRP2','usb',1);
%c=invoke(IO.r, 'ConnectRZ6','GB',1);
%l=invoke(IO.r,'LoadCOF',[IO.Root,IO.rcoFile]);	% load .RCO file	
...
%Find ID for masking speakers
speakerIDMask = audiodevinfo(0,'Line 5/6 (M-Audio M-Track Eight) (Windows DirectSound)');       %% Finds the correct output device indentifier for the masking speakers. 
LevScalar = (10^(TRACK.MaskerLev/20))/(10^(83/20));
%LevScalar = 1
% Switch on the selected masker
switch STIM.MaskerCond
    case 'Experiment 1'                                                                     %%Experiment 1 
        LevScalarL = (10^(TRACK.MaskerLev/20))/(10^(83/20));
        [y1, Fs1] = audioread('JackAndTheBeanStalk_Male1_SSN.wav');                         %%Read in data for background noise file
        yarray1 = zeros(5*length(y1),2);                                                      
        yarray1(:,1) = [y1;y1;y1;y1;y1];                                                    %%Store background noise info in channel 1 of 2-channel array
        yarray1 = yarray1*LevScalarL;
        player1 = audioplayer(yarray1,Fs1,16,speakerIDMask);                                %%Create audioplayer object to play noise
        
        LevScalarR = (10^(TRACK.MaskerLev/20))/(10^(83/20));
        [y2, Fs2] = audioread('JackAndTheBeanStalk_Male2_SSN.wav');                         %%Read in data for background noise file
        yarray2 = zeros(5*length(y2),2);
        yarray2(:,2) = [y2;y2;y2;y2;y2];                                                     %%Store background noise info in channel 2 of 2-channel array
        yarray2 = yarray2*LevScalarR;
        player2 = audioplayer(yarray2,Fs2,16,speakerIDMask);                                    %%Create audioplayer object to play noise
                        
    case 'Experiment 2'                                                                     %%Experiment 2
        LevScalarL = (10^(TRACK.MaskerLev/20))/(10^(83/20));
        [y1, Fs1] = audioread('JackAndTheBeanStalk_Male1_SSN.wav');                         %%Read in data for background noise file
        yarray1 = zeros(5*length(y1),2);                                                      
        yarray1(:,1) = [y1;y1;y1;y1;y1];                                                    %%Store background noise info in channel 1 of 2-channel array
    ...
    case 'Experiment 3'                                                                     %%Experiment 3
        LevScalarL = (10^(TRACK.MaskerLev/20))/(10^(83/20));
...
%------------------------------------------------------------------%
% UpdateMaskLev: Update sound level of maskers.                    %
%------------------------------------------------------------------%
function [player1,player2]=UpdateMaskLev(y1,y2,Fs1,Fs2,speakerIDMask,TRACK,ti1,ti2)

LevScalarL = (10^((TRACK.MaskerLev)/20))/(10^(56.5/20));
yarray1 = zeros(5*length(y1),2);                                                      
yarray1(:,1) = [y1;y1;y1;y1;y1];       
yarray1 = yarray1*LevScalarL;
...
if start1>length(y1)
    start1 = start1-length(y1);
end

play(player1,start1);

LevScalarR = (10^((TRACK.MaskerLev)/20))/(10^(56.5/20));
yarray2 = zeros(5*length(y2),2);                                                      
yarray2(:,2) = [y2;y2;y2;y2;y2];       
yarray2 = yarray2*LevScalarR;
...
if start2>length(y2);
    start2 = start2-length(y2);
end

play(player2,start2);
%CML = 20*log10(LevScalar*10^3);                    
%disp('Calculated Masker Level:  '); disp(CML)      These two lines were
%included to verify the soundlevel calculations are correct.
end
...
%[y, Fs]=audioread('JackAndTheBeanStalk_Male1_SSN.wav');
ystim=y';
padpts=STIM.WordNumPts-length(ystim);
FullArray=[ystim,zeros(1,padpts)];
%e = invoke(IO.r, 'WriteTagV', 'Token_in', 0, FullArray); 
%if(~e), error('Error writing to buffer'); 
%end
figure(5);
plot(FullArray); 
drawnow;
...
switch STIM.MaskerCond
    case 'Experiment 1'                                                                     %%Experiment 1 
        speakerIDStim = audiodevinfo(0,'Line 7/8 (M-Audio M-Track Eight) (Windows DirectSound)');  %%Find ID for speaker directly in front of the patient (Exp 1)
                       
    case 'Experiment 2'                                                                     %%Experiment 2
        speakerIDStim = audiodevinfo(0,'Line 1/2 (M-Audio M-Track Eight) (Windows DirectSound)');
                
    case 'Experiment 3'                                                                     %%Experiment 3
        speakerIDStim = audiodevinfo(0,'Line 7/8 (M-Audio M-Track Eight) (Windows DirectSound)');  %%Find ID for speaker directly in front of the patient (Exp 3)
                
    otherwise
        error('Error playing stimulus over selected speaker.');
end


yarray = zeros(length(y),2);
yarray(:,1) = y;
SigScale = (10^(STIM.TargetSigLev/20))/(10^(42/20));
%SigScale = 1
yarray = SigScale*yarray;
playerstimulus = audioplayer(yarray,Fs,16,speakerIDStim);
...
%------------------------------------------------------------------%
function [Resp,RespRight,TRACK]=GetResp(TRACK)
...
while isempty(RespRight),
    ...
    if strcmp(Resp,TRACK.AllPlayed{end}),
        ...
        TRACK.Correct(TRACK.trialnum) = {'y'};
    else
        ...
        TRACK.Correct(TRACK.trialnum) = {tmp};
    end;
end;
...
eval([TimeStamp,'.Correct=TRACK.Correct;']);

cd([IO.Root,'data']);
eval(['save ',TimeStamp,' ',TimeStamp,'']);

filename = [TimeStamp,'.xlsx'];
fileID = fopen([IO.Root,'data']);

%expdata = zeros(TRACK.trialnum,5);

Reversals = TRACK.RevLevel;
Reversals = Reversals';

AllLevel = TRACK.AllLevel;
AllLevel = AllLevel';
%AllLevel = num2cell(AllLevel);

AllResp = TRACK.AllResp;
AllResp = AllResp';
%AllResp = num2cell(AllResp);

AllPlayed = TRACK.AllPlayed;
AllPlayed = AllPlayed';
%AllPlayed = num2cell(AllPlayed);

Correct = TRACK.Correct;
Correct = Correct';
%Correct = num2cell(Correct);

%C = mat2cell(expdata,[TRACK.trialnum],[1 1 1 1 1]);
ExpName = 'Boystown';

title = {'ExperimentName','Tester','Masker','Threshold','Reversals', 'All Levels','All Responses','Stimuli Played','Correct?'};
data1 = {ExpName,ObsName, STIM.MaskerCond, thresh};
data2 = num2cell(Reversals);

data3 = cell(TRACK.trialnum,4);
data3(:,1) = num2cell(AllLevel);
data3(:,2) = AllResp;
data3(:,3) = AllPlayed;
data3(:,4) = Correct;

xlswrite(filename,title,'B1:J1')
xlrange1 = ['B2:E',num2str(1+TRACK.trialnum)];
xlrange2 = ['F2:F',num2str(1+length(data2))];
xlrange3 = ['G2:J',num2str(1+TRACK.trialnum)];

xlswrite(filename,data1,xlrange1)
xlswrite(filename,data2,xlrange2)
xlswrite(filename,data3,xlrange3)
end
%------------------------------------------------------------------%
